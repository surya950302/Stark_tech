<?php  
  ...
  if (mysqli_connect_errno())
  {
   ...
   echo "Start date is ".$start." <br/> till ".$till;
      ...
      $response=mysqli_query($conn, "SELECT MAX(y.X),y.PID FROM (SELECT PID,COUNT(PID) AS X FROM cart  JOIN appears_in ON cart.cartid=appears_in.cartid  WHERE TDATE between '$start' AND '$till' group by PID) as y ");
      ...
      {        
        ...
        $str.= "<tr><td><h3>"."Number of times product sold: ". $row['MAX(X)']."</h3></td></tr><tr><td><h3>"."Most Frequently Sold Product ID: ".$row['PID']."</h3></td></tr>";
      }
        $str.= "</table>";
      }
      return $str;
